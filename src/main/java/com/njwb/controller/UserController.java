...
//	    	request.getRequestDispatcher("/njwb/result.jsp").forward(request, response);
	...
	public String editPwd(HttpServletRequest request, HttpServletResponse resp) throws UnsupportedEncodingException, FileUploadException, Exception {
	 ...
	 * @throws IOException 
	 */
	public void exit(HttpServletRequest request, HttpServletResponse resp) throws IOException {
	...
	public String exit2(HttpServletRequest request, HttpServletResponse resp) {
	...
	public String queryAllUsByPage(HttpServletRequest req,
	...
	public void delUser(HttpServletRequest request,
		...
		String result = "success";
			...
			try {
				count = userService.deleteByNo(Integer.parseInt(userNo));
	 ...
	 */
	public String addUser(HttpServletRequest request, HttpServletResponse resp) throws UnsupportedEncodingException, FileUploadException, Exception {
		//首先判断是否为二进制提交
		if(!ServletFileUpload.isMultipartContent(request))
		{
			log.info("请求不是二进制提交，请检查表单的enctype,method");
	...
	public String userDetail(HttpServletRequest req, HttpServletResponse resp) {
		HttpSession session = req.getSession();
		String userNo = req.getParameter("userNo");
		User user;
		List<Role> rolelist= roleService.queryAllRole();
		try {
			user = userService.queryByUserNo(Integer.parseInt(userNo));
		} catch (OAException e) {
	 ...
	 * @throws Exception
	 */
	public String editUser(HttpServletRequest request, HttpServletResponse resp) throws UnsupportedEncodingException, FileUploadException, Exception {
		if(!ServletFileUpload.isMultipartContent(request))
		{
			log.info("请求不是二进制提交，请检查表单的enctype,method");
			request.setAttribute("resultCode", ErrorCode.ERROR_User);
